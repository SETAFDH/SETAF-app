        ...
        return
            switch ($field)
                ...
                default return
                    if (exists($target-texts)) then
                        for $text in $target-texts
                        let $divisions := $config:data-root ! doc(. || "/" || $text)//tei:div//tei:choice/tei:reg[@type='intermediary'][ft:query(., $query, query:options($sortBy))]
                        return
                            if (empty($divisions)) then
                                $config:data-root ! doc(. || "/" || $text)//tei:text//tei:choice/tei:reg[@type='intermediary'][ft:query(., $query, query:options($sortBy))]
                            else
                                $divisions
                    else
                        let $divisions := collection($config:data-root)//tei:div//tei:choice/tei:reg[@type='intermediary'][ft:query(., $query, query:options($sortBy))]
                        return
                            if (empty($divisions)) then
                                collection($config:data-root)//tei:text//tei:choice/tei:reg[@type='intermediary'][ft:query(., $query, query:options($sortBy))]
                            else
                                $divisions
    else ()
};
...
declare function teis:autocomplete($doc as xs:string?, $fields as xs:string+, $q as xs:string) {
    ...
    return
        switch ($field)
            ...
            case "text" return
                if ($doc) then (
                    doc($config:data-root || "/" || $doc)/util:index-keys-by-qname(xs:QName("tei:reg"), $lower-case-q,
                        function($key, $count) {
                            $key
                        }, 30, "lucene-index"),
                    doc($config:data-root || "/" || $doc)/util:index-keys-by-qname(xs:QName("tei:reg"), $lower-case-q,
                    ...
                    collection($config:data-root)/util:index-keys-by-qname(xs:QName("tei:reg"), $lower-case-q,
                        function($key, $count) {
                            $key
                        }, 30, "lucene-index"),
                    collection($config:data-root)/util:index-keys-by-qname(xs:QName("tei:reg"), $lower-case-q,
