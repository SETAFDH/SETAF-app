...
(: Map the text of the static pages with the language of the website :)
declare function mapping:change-language($root as element(), $userParams as map(*)) {
    let $language := $userParams?language
    return if ($language = 'en')
            then $root//tei:div[@xml:lang="en"]
        else $root//tei:div[@xml:lang="fr"]
};

declare function mapping:change-breadcrumb($root as element(), $userParams as map(*)) {
    let $language := $userParams?language
    return if ($language = 'en')
            then $root//tei:title[@xml:lang="en"]
        else $root//tei:title[@xml:lang="fr"]
};
