...
declare variable $param := request:get-parameter('collection', '');
...
if ($exist:path eq '') then
    ...
    (: forward root path to accueil.html :)
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="accueil.html"/>
    </dispatch>
...
(: Redirection of the default "gravures" index to the page index_gravures.html :)
else if ($param = 'gravures') then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{$exist:root}/{$exist:controller}/index_gravures.html"/>
    </dispatch>
